...
const DoctorChat = ({ appointmentId, userId, docId, backendUrl, dToken, patientName, patientImage }) => {
  ...
  const loadMessages = async () => {
    try {
      const { data } = await axios.post(
        `${backendUrl}/api/chat/doctor/messages`,
        { appointmentId },
        { headers: { dToken } }
      );
  ...
  const uploadFile = async () => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post(
        `${backendUrl}/api/chat/doctor/send-file`,
        formData,
        {
          headers: {
            dToken,
  ...
  return (
    <div className="flex flex-col h-[500px] border rounded-lg bg-white">
      ...
      <div className="p-3 border-b flex items-center gap-2 bg-gray-50">
        <img src={patientImage} alt={patientName} className="w-8 h-8 rounded-full" />
        ...
        {messages.length === 0 ? (
          ...
          messages.map((message) => (
            ...
            >
              ...
              <div className={`text-xs mt-1 ${message.senderId === docId ? 'text-white/70' : 'text-gray-500'}`}>
